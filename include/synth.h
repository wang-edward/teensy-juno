...
class synth {
  public:
    ...
    static const int debounce_time =
        5; // amount of time between checks, not total time
    ...
    parameter *parameters[number_parameters] = {&lfo_rate, &lfo_delay,
                                                &osc_lfo_level, &master_volume};

    button *buttons[number_buttons] = {
        &pulse_on,          &saw_on,      &pwm_lfo_on,
        &envelope_polarity, &envelope_on, &flanger_on,
        &velocity_on,       &write,       &reset};
